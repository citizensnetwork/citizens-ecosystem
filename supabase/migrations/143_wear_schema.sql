-- ============================================================================
-- 143_wear_schema.sql — wear.* schema  (Citizens Wear, ecosystem Step 3)
-- ============================================================================
-- Activates the THIRD schema boundary the shared-DB contract reserved
--   (`public`/commons, `vision.*`, **`wear.*`**). Authored in the
--   citizens-connect migration lineage (single source of truth for the shared
--   project `xyiajtrvhlxaeplsiajj`) per ADR-0007 §2.
--
-- MODEL (Direction A, founder-ratified — STEP3_WEAR_INTEGRATION_SCOPE §2):
...
--     DISPLAY-SAFE identity (Q1), keyed by the auth uid — NO email/PII column
--     (email lives in auth.users, readable only by its owner via the session).
...
--
-- FAITHFUL to citizens-wear packages/db/prisma/schema.prisma, with the changes
-- the real DB requires: (a) uuid PKs + auth.users-backed identity; (b) RLS on
-- every table; (c) no public email column; (d) two SECURITY DEFINER helpers to
-- break the recursive-RLS trap on membership (mirrors Connect mig 135) and to
-- evaluate mutual blocks (a target must not be able to read that it was
-- blocked). `wear.products` is intentionally DEFERRED — the current Wear model
-- references Connect product ids via posts.tagged_product_ids[]; a first-class
-- catalog is a later product decision.
-- ============================================================================
...
-- ── Identity (DISPLAY-SAFE mirror of the shared auth.users, Q1) ─────────────
...
-- No email column: email is PII and this table is public-SELECT.
...
-- ── Direct messages ─────────────────────────────────────────────────────────
-- Tables are created BEFORE the SECURITY DEFINER membership helper, because a
-- `language sql` function body is validated at CREATE time (check_function_bodies
-- is on by default) — the helper cannot reference a table that does not yet
-- exist. Policies that call the helper come last.
...
-- Created here, after wear.conversation_members exists.
...
-- Mutual-block check — SECURITY DEFINER because a target must NOT be able to
-- read the row proving they were blocked (blocks_actor_read hides the reverse
-- direction). The store's isBlockedEither() calls this instead of a direct
-- select so both directions are evaluated without leaking either row.
create or replace function wear.is_blocked_either(p_a uuid, p_b uuid)
returns boolean language sql security definer stable set search_path = '' as $$
  select exists (
    select 1 from wear.blocks b
    where (b.actor_id = p_a and b.target_id = p_b)
       or (b.actor_id = p_b and b.target_id = p_a)
  );
$$;
revoke all on function wear.is_blocked_either(uuid, uuid) from public;
grant execute on function wear.is_blocked_either(uuid, uuid) to authenticated, service_role;
...
-- ============================================================================
-- Post-apply checklist: get_advisors(security)=0 ERROR; expose `wear` schema in
-- PostgREST "Exposed schemas"; smoke-test RLS (a user cannot read another
-- user's private saves / settings / others' blocks / reports / followers-only
-- stories).
-- ============================================================================
