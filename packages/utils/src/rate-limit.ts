 ...
 *
 * Canonical `@citizens/utils` copy, extracted verbatim from Citizens
 * Connect's `src/lib/rate-limit.ts` (ecosystem decision brief row 4; Vision's
 * port was kept byte-compatible with Connect's on purpose so all three apps
 * consolidate onto this module). Wear is the first workspace consumer.
 *
...
function cleanupInMemory(windowMs: number): void {
  const now = Date.now();
