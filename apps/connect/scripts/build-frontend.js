...
copyDir(SRC, DEST);
console.log('[build-frontend] Copied src/frontend/ → public/');

// Generate config.js. API_BASE_URL is empty when the frontend is served from
// the same domain as the API (the standard Vercel deployment topology).
const cfg = {
  SUPABASE_URL:     process.env.NEXT_PUBLIC_SUPABASE_URL      || '',
  SUPABASE_ANON_KEY:process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '',
  API_BASE_URL:     process.env.NEXT_PUBLIC_API_BASE_URL       || '',
  MAPTILER_KEY:     process.env.NEXT_PUBLIC_MAPTILER_KEY       || '',
  MAPTILER_STYLE:   process.env.NEXT_PUBLIC_MAPTILER_STYLE     || 'streets-v2',
};
fs.writeFileSync(
...
console.log('[build-frontend] Generated public/config.js');
