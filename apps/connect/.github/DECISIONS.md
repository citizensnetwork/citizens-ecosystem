...
### Map update bubbles are public, time-boxed, and an isolated marker layer
**Decision:** Event updates and event broadcasts auto-create a `map_bubbles` row (snippet ≤160 chars, `expires_at = now() + 24h`) via AFTER-INSERT SECURITY DEFINER triggers (`tg_bubble_from_event_update`, `tg_bubble_from_broadcast`; the latter fires for `entity_type='event'` only). Both trigger functions have EXECUTE revoked from public/anon/authenticated so they are never callable over the REST RPC surface. Reads go through `get_active_map_bubbles()` (SECURITY DEFINER, anon+authenticated) which returns only live bubbles the caller has not dismissed; anonymous callers (auth.uid() null) get all live bubbles. `dismiss_map_bubble(uuid)` records a per-user dismissal in `bubble_dismissals` idempotently. RLS: bubbles are publicly readable only while `expires_at > now()` (no client writes); dismissals are own-row only. The POST `/api/map/bubbles/[id]/dismiss` route adds auth + UUID + rate-limit. In the client, EventsView fetches via `rpc("get_active_map_bubbles")`, keeps at most one (newest) bubble per event, and EventMap renders them as a **separate marker layer** with its own ref and zoom listener — deliberately not entangled with the event-marker deconfliction/culling system — revealed only at `BUBBLE_MIN_ZOOM` (12) and anchored above each event marker. Bubble text is `escapeHtml`-sanitised in `createBubbleEl`.
**Why:** Spec (`docs/feature-clarity/map-layering.md` + `notifications.md`): update bubbles are public visual invitations on the map (valuable z12+, ~24h life, dismissible per viewer), not private notifications. A public read of active-only rows plus identity-free anonymous reads matches "any passer-by sees it"; per-user dismissals reduce clutter without making the bubble private. Keeping the bubble layer isolated protects the fragile marker deconfliction logic from regressions.
**Date:** Batch notif-matrix-4.

### Anonymous broadcast reactions are aggregate-only
**Decision:** Broadcast reaction counts live in `broadcast_reactions(broadcast_id, emoji, count)` — a PK of `(broadcast_id, emoji)` with no user id column. The five allowed emoji are the single constant `BROADCAST_REACTION_EMOJI` in [src/lib/broadcasts.ts](src/lib/broadcasts.ts), enforced identically by a table check constraint, the `increment_broadcast_reaction` SECURITY DEFINER RPC, and the POST `/api/broadcasts/[id]/react` route. Clients cannot write the table directly (no INSERT/UPDATE/DELETE policy, SELECT is public); all bumps go through the RPC, which validates auth + emoji + that the broadcast is live. The route adds auth + UUID + rate-limit. `OrgBroadcastList` renders `BroadcastReactions` only when `showReactions` is set — event views only, not places.
**Why:** Spec (`docs/feature-clarity/notifications.md`): reactions must be aggregate counts only, never identity-bearing social pressure. Storing no user id makes that structural rather than a UI choice; abuse is bounded by auth + rate-limit, which is acceptable for an identity-free vibe signal. One emoji constant prevents the client/server lists drifting.
**Date:** Batch notif-matrix-3.

### Material-change push filtering for event updates
**Decision:** Event-update notifications split delivery by RSVP status and materiality. `event_updates.is_material boolean default false` (migration 127) marks push-worthy updates: date/time, location/venue, and volunteer openings (price is out of scope — events have no price field). The field-change trigger `notify_event_field_changes()` now also watches `volunteer_openings` and stamps every synthetic row `is_material = true`; manual organiser posts default to `false`. `notify-event-update` (v2) delivers the **in-app** bell to every pref-allowed recipient (attending + considering, minus per-event opt-outs), but delivers **push** to all allowed recipients only when `is_material`, otherwise to attending RSVPs only. Implemented via two `sendNotifications` calls using the new `skipPush` (in-app only) and existing `skipInApp` (push only) flags on `_shared/push.ts`. Cancellations remain on the separate `notify-event-cancelled` path.
**Why:** Matches the notifications spec: considering users get a quieter signal (in-app) for routine edits but a push for changes that affect whether/when they can attend. Splitting the two `sendNotifications` calls keeps `push.ts` a single source of FCM logic while letting the caller choose the channel per recipient set.
**Date:** 2026-06-02, notif-matrix Batch 2.
