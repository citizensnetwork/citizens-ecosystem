...
|-------|-------|--------|-------|
| polish-2 | `/audit-polish 3` — Polish Queue rows 4-6 (places-browse-and-follow + notifications + events-browse) | **Shipped** `0d609c5` | 8 Report-only items across 3 surfaces. **Surface 4 places-browse-and-follow:** (a) `GET /api/manage/places` gains user-scoped rate-limit (`RATE_LIMITS.read`); (b) `FollowPlaceButton` surfaces 429 + generic fetch failure in a `role="alert"` region (mirrors `RSVPButton`); (c) `ManagePlacesView` adds error state + retry button; (e) UUID validation dedup — replaced inline `UUID_RE.test(...)` with the centralised `isValidUUID()` helper across `admin/users`, `admin/api-keys` (×2), `admin/pending-elevations/[id]/approve`, `v1/events`, `v1/events/[id]` (net security tightening — helper adds `typeof === "string"` guard). **Surface 5 notifications:** (b) dismissed-review-prompts localStorage list capped at 100 entries with FIFO eviction; (c) `NotificationBell` realtime subscription extended with UPDATE + DELETE postgres_changes handlers alongside INSERT (filtered `user_id=eq.${userId}`) — read-state and dismissals stay in sync across tabs/devices. **Surface 6 events-browse:** (a + b) `/events` data-loading restructured — `getUser()` resolves first (fast cookie read), then a single `Promise.all` fires publicEvents + places (`.limit(1000)`) + reviews (`.gte(created_at, twelveMonthsAgo).order(created_at desc).limit(5000)`) + contributors + preferencesPromise. Removes the sequential preferences round-trip and bounds two previously unbounded queries. New module constants `PLACES_LIMIT` / `REVIEWS_LIMIT` / `REVIEW_WINDOW_MONTHS` with rationale comments. **Deferred per user choice:** 4d (manage-places SQL aggregate — needs migration), 5a (PendingReviews realtime dedup — low impact), 6c (events detail panel extraction — refactor). 714 tests passing; `tsc` 0; `next lint --dir src` clean; advisors 83 → 83 (code-only). Architect SE: A across all three surfaces — SHIP, no Should-fix; 4 nice-to-haves logged (AbortController on ManagePlacesView fetch, REPLICA-IDENTITY comment on DELETE handler, distinct network-error copy on FollowPlaceButton, per-place review-count truthness when reviews truncate at 5000). |
| polish-1 | `/audit-polish 3` — Polish Queue rows 1–3 (onboarding column drop + event-detail metadata cache + profile UUID guards) | **Shipped** (pending commit) | Migration `093_drop_profiles_onboarding_completed.sql` (file-only — idempotent `do $$ … drop column if exists … end $$`; awaits normal Supabase cadence to apply to remote). Removed all reads of `profiles.onboarding_completed` from `schema.sql`, `src/types/db.ts`, `src/app/profile/page.tsx`, `src/__tests__/helpers/fixtures.ts`, `src/__tests__/lib/aiSearch.test.ts`, and 6 dead `update` lines in `supabase/migrations/061_seed_testing_contributors.sql` (architect Should-fix — prevents replay failure after 093). `EventDetailContent.tsx`: `MessageButton recipientName` now `organiser?.full_name || "Organiser"` instead of hard-coded `"Organizer"`. `src/app/events/[id]/page.tsx` `generateMetadata` now reuses `getEventById` (React `cache()` from `EventDetailServer`) so metadata + body render share one Supabase round-trip per request. `src/app/profile/[id]/[mode]/page.tsx` uses `isValidUUID` from `@/lib/validation` instead of local `UUID_RE` regex. `src/app/profile/[id]/page.tsx` adds early `isValidUUID(id)` guard in both `generateMetadata` (returns `{ title: "Profile Not Found" }`) and the page body (calls `notFound()`) before any Postgres round-trip. `RESUME_HERE.md` debt bullet removed; `.audit/QUEUE.md` rows 1–3 marked shipped; surface checkpoints append "Polish run 2026-05-23" section. 714 tests passing; `tsc` 0; `next lint --dir src` clean; advisors 83 → 83 (code-only — migration is file-only). Architect SE: 2 Should-fixes applied inline (061 cleanup + QUEUE.md state). |
| 14h | Audit fix — P1 notifications + P2 map-core (final two findings-ready surfaces) | **Shipped** `01ec87a` + `ff4d9f5` | `NotificationBell.tsx`: optimistic revert pattern in `handleMarkRead`/`handleMarkAllRead`/`handleDelete` — captures `prevState` before `setNotifications`, restores it when `fetch` rejects or returns `!ok` (network / 429 / 500). Realtime channel name scoped per user (`notifications:${userId}` instead of bare `"notifications"`) so a same-tab logout/login of a different user cannot reuse a stale subscription bound to the previous `user_id`. `LocationPicker.tsx`: reverse-geocode now uses `AbortController` (cancels the previous in-flight Nominatim fetch on each new click and on unmount) so rapid clicks no longer race — latest click wins. Privacy disclosure line added beneath the picker informing the user that pinned coordinates are sent to `nominatim.openstreetmap.org` for street-name lookup. 714 tests; `tsc` 0; `next lint --dir src` clean; advisors 83 → 83 (code-only — neither surface touches DB). Inline architect + security reviews (both Tier A, low-risk client UX); no Should-fix. Three staged patches archived to `.audit/patches/applied/`. With this batch the staged audit queue is empty — next run is `/audit` (pick a new surface) or `/audit-polish 1` (start the Polish Queue). |
| QP1 | Quick-search panel: tab-gated tiles + city chips + proximity sort | **Shipped** `11372b9` | Quick-search panel used to render BOTH event and place tiles regardless of burgerTab. Because place markers hide below zoom 12 on the map, tapping a place tile flew the map to an empty patch. Now gated: `events` tab → event tiles only; `places` tab → place tiles only. Empty-state, header count, carousel pagination + touch-swipe math all use the gated total. New `src/lib/cityLabel.ts` resolves short city codes (PTA / JHB / CT / DBN / PE / BFN / ELS / PMB / POL / GRG / STB / RBT / KIM) via padded text match then 40 km haversine fallback, rendered as a chip on every event/place tile in both the category panel and quick-search panel. Tiles sort closest-first using cached `userLocation` (no new geolocation prompt) with `DEFAULT_CENTER` (Pretoria) as fallback. `quickPanelPage` resets to 0 on tab/category/search change so the carousel never sits past the end of a shorter result set. `distanceKm` now exported from `aiSearch.ts` and reused. New memo `sortedCategoryPanelEvents` keeps proximity ordering off the unsorted `filtered` array that feeds map markers. 11 new tests in `cityLabel.test.ts` including bare-`Stellenbosch`→`STB` regression, padded-boundary check (`Capricorn Park` must not match `CT`), and text-wins-over-coords priority. 714 tests passing (+11); `tsc` 0; `next lint --dir src` clean; advisors unchanged (code-only). Architect SE: A after applying both Should-fix items inline (STB ordering + `quickPanelPage` reset). |
...
| 11 | BUG: Admin contributor approvals 500 — notifications column fix + deep-link wiring | **Shipped** `cf9e00b` | Postgres `column "url" of relation "notifications" does not exist` raised on every admin approve/reject click. Migration 084 rewrites `approve_contributor_application` + `reject_contributor_application` RPCs to insert into `data` jsonb (`jsonb_build_object('url', '/profile/contributor')`) instead of the non-existent `url` column; both gain `set search_path = pg_catalog, public`. Edge function `review-contributor-application` v3 deployed via MCP — service-mode fallback in `performReviewAsService` now writes `data: { url: ... }` too. `NotificationPanel.getNotificationLink` extended with a `data.url` branch (architect must-fix) so contributor approval notifications are actually clickable. 697 tests; advisors 0 ERROR / 82 WARN (unchanged baseline — the 4 SD-function warnings on these RPCs were pre-existing since migration 036). |
| 12 | BUG: Contributor approve/reject still 500 + Delete control for admin queue | **Shipped** | Root cause = `notifications.type` CHECK constraint never included `contributor_approved` / `contributor_rejected` (introduced by migration 084 RPCs). Edge function still returned 500 because every notification insert hit `notifications_type_check`. Migration 085 widens the constraint to include both contributor types, and adds an admin-only SECURITY DEFINER `delete_contributor_application(uuid)` RPC that hard-deletes an application and resets `profiles.contributor_status` from `pending` → `not_applied` so the user can re-apply. New `POST /api/admin/contributors/delete` (requireAdmin + UUID validation + mutation rate-limit + audit log). `ContributorReviewCard` gains a red Delete button with confirm-step UI. 697 tests pass; advisors no new findings. |
| 14g | Audit fix — P2 messaging-dm (maybeSingle parity + read rate-limit + NaN guard) | **Shipped** `94dc675` | 5× `.single()` → `.maybeSingle()` across `conversations/route.ts` (recipient lookup) and `conversations/[id]/messages/route.ts` (participant verify ×2, cursor msg lookup, other-participant lookup) — RLS-denied / missing-row reads now return clean `{data:null}` instead of swallowed PGRST116. `GET /messages` `?limit=` parsing now NaN-safe (`Number.isFinite` + `>0` guard, default 50, cap 100). `PATCH /conversations/[id]/read` adds rate-limit gate (`conv-read:${user.id}`, `RATE_LIMITS.read` = 120/min) with `Retry-After` header on 429. 9 test mock sites flipped to `maybeSingle.mockResolvedValueOnce`. 703 tests passing; `tsc` 0; `next lint --dir src` clean; advisors 83 → 83 (code-only). Architect SE: A — APPROVED, no Must/Should-fix; 4 nice-to-haves deferred (scope cursor `before` to conv_id, `has_more` off-by-one, rate-limit before participant verify on POST, document read PATCH non-disclosure posture). Three staged patches archived. |
| 14f | Audit fix — P1 place-create-edit-media (length CHECKs + 6-month delete trigger + native-confirm cleanup) | **Shipped** `2906189` | Migration 088: `places` length CHECKs (name 1..120, description 0..4000, address 0..500, phone ≤32, website ≤500, custom_category ≤120) added `NOT VALID` and validated with `EXCEPTION WHEN check_violation` so existing-row violations don't block deploy. Migration 089: BEFORE DELETE trigger `enforce_place_six_month_delete()` (SECURITY DEFINER + admin bypass via `is_admin()`) — raises `errcode 42501` "Places can only be deleted after 6 months. Edit instead." Migration 090: lower-bound fix on description/address (was rejecting column's `default ''`). Migration 091: `REVOKE … FROM anon, authenticated` on the trigger function to satisfy Supabase advisors (trigger invocation unaffected by EXECUTE revoke). schema.sql Section 6 mirrors all four. `CommentSection`: delete errors now captured in `deleteError` state and surfaced in `ConfirmModal` instead of swallowed. `EditPlaceForm.handleRemoveExistingMedia` made sync (no false `async` without `await`). 703 tests passing; `tsc` 0; `next lint --dir src` clean; advisors 83 → 85 after 089 → 83 after 091 (back to baseline). Architect SE: 2 Should-fixes applied inline (bounds fix, error surfacing). Three staged patches archived. |
| 14e | Audit fix — P2 event-create-edit (boundary validation + delete error surfacing) | **Shipped** `71c9085` | `EventForm.tsx` + `EditEventForm.tsx` `handleSubmit` gain a shared boundary-validation block right after `setLoading(true)`: trims title (rejects empty), validates `coords` are finite and within lat/lng ranges, and rejects `endTime <= date`. Each early-return clears `loading`. `.insert({...})` / `.update({...})` now pass `title: trimmedTitle`. `EditEventForm.handleDelete` captures `delErr` from `.delete()` and surfaces `"Failed to delete event: ..."` instead of silently redirecting on RLS denial or FK violation. `.gitignore` extended to exclude `.vitest*.log`. 703 functional tests (1 unrelated Navbar timeout flake under full-suite load — passes in isolation); `tsc` 0; `next lint --dir src` clean; advisors 83 → 83 (code-only). Architect SE: A — SHIP, no Should-fix; nice-to-haves (DB CHECK constraints, shared validator helper, title length cap) deferred. Two staged patches archived. |\n| 14d | Audit fix — P1 edge-functions (push fan-out attending-only + payload type union) | **Shipped** `7fba70d` | `send-rsvp-reminders/index.ts`: rsvps batch query gains `.eq("status","attending")` (was leaking to `considering`). `send-event-reminder/index.ts`: same `.eq("status","attending")` filter + new `filterUserIdsByPref(supabase, rawUserIds, "event_reminders")` per-event call so notification preferences are honoured (parity with rsvp-reminders). `prompt-post-event-reviews/index.ts`: `.eq("status","attending")` on the per-event rsvps query inside `Promise.all`. `_shared/push.ts`: `PushPayload.type` union widened with `"review_prompt" \| "contributor_approved" \| "contributor_rejected"` (all three already in DB `notifications.type` CHECK from migration 085, so backward-compatible). 703 tests; `tsc` 0; `next lint --dir src` clean; advisors 83 → 83 (code-only). Architect SE: A — SHIP, no Should-fix; 3 nice-to-haves noted (hoist per-event pref fetch into one batch query, comment-drift cleanup, document `event_reminders` covers review prompts so a separate pref key isn't introduced). Four staged patches archived. |\n| 14c | Audit fix — P1 auth-and-signup (PostgREST filter-injection + login redirect) | **Shipped** `e38fca5` | `/api/indemnity` GET: `applies_to` query param validated against `["events","places","both"]` allowlist before interpolating into the PostgREST `.or()` filter on `indemnity_templates` — closes filter-injection vector. `LoginForm.tsx`: respects middleware's `?redirect=` query param via new `safeRedirect()` guard (must start with single `/`, no `\`, no `:`), mirroring `auth/callback` open-redirect protection. `postLoginPath` threaded through to `OAuthButtons` (the `redirectTo` prop already existed and the PKCE callback already validates `next` — defense in depth). 703 tests passing; `tsc` 0 errors; `next lint --dir src` clean; advisors 83 → 83 (code-only). Architect SE: A — no should-fix. Two staged patches archived to `.audit/patches/applied/`. |
| 14b | Audit fix — P1 storage-and-media-uploads (SVG-XSS + ProfileEditor RLS) | **Shipped** `bc83f3c` | Dropped `image/svg+xml` from `ALLOWED_IMAGE_TYPES` / `SAFE_IMAGE_EXTENSIONS` / `imageCompression.SKIP_TYPES` and every form `accept=` attr (events × 2, places × 2, media gallery) — closes XSS on public buckets. ProfileEditor `handleAvatarUpload` now uses canonical `validateImageFile` + `compressImageIfNeeded` + `safeImageExtension`, and writes to `${profile.id}/avatars/${ts}.${ext}` so the storage RLS predicate `(storage.foldername(name))[1] = auth.uid()::text` is actually satisfied (previously RLS-broken). Architect verdict: ship as-is — 4 nice-to-haves folded into deferred storage-hygiene line (legacy SVG purge, shared `IMAGE_ACCEPT` constant, dedicated avatars bucket, accept-attr ordering cosmetic). 703 tests; `tsc` 0 errors; `next lint --dir src` clean; advisors 83 → 83 (code-only). |\n| 14a | Audit fix — P1 rsvp-and-comments (CRITICAL safe_rsvp IDOR) | **Shipped** `fa1ac6b` | Migration 086: `safe_rsvp` hardened — `auth.uid() = p_user_id` guard, `search_path = pg_catalog, public`, REVOKE EXECUTE FROM public + GRANT TO authenticated. Mirrors `toggle_consider` (070). Migration 087: `comments.body` 1..2000 CHECK constraint (`NOT VALID + VALIDATE`) + `comments_user_id_idx`. `RSVPButton.tsx`: surfaces 409/429/500 via `<p role="alert">` instead of silent swallow; maps 429 to friendly fallback; preserves 401-redirect. +3 RSVPButton regression tests (capacity-full / rate-limited / DELETE failure). schema.sql mirror updated. Carryover commit `0c6f0b5` first batched 8 prior `.single() → .maybeSingle()` Fix-clean edits + audit-infra updates onto main. 704 tests passing (+3); `tsc` 0 errors; `next lint --dir src` clean; advisors 84 → 83 (`function_search_path_mutable` on safe_rsvp closed; no new warnings). Architect SE: ✅ ship as-is. |
| 13 | Map perf — basemap layer pruning + MapTiler "Lite" checklist + DOM marker culling | **Shipped** | Three-pronged map perf pass. (1) **Runtime basemap pruner** in `src/lib/map/config.ts`: `pruneBasemapLayers(map)` walks `map.getStyle().layers` after `style.load` and removes POIs, buildings, transit, ferry/aeroway, housenumbers, hillshade, contours — works on the current MapTiler style without a Studio rebuild. Toggled via `NEXT_PUBLIC_MAP_PRUNE` (defaults on). Wired into `EventMap`, `MiniMap`, `MapBackdrop`, `LocationPicker`. (2) **`docs/MAP_TILER_LITE_CHECKLIST.md`** — durable Studio recipe for the "Citizens Connect Lite" custom style (layers to drop / keep, font/glyph trims, palette, env-var swap, rollback). User has the new Lite UUID in `.env.local`; **Vercel Production + Preview env vars (`NEXT_PUBLIC_MAPTILER_STYLE`) still need a manual update** to take effect on the deployed site. (3) **DOM marker culling** in `EventMap.tsx` — `cullMarkers()` toggles `display:none` on event/place markers whose `lngLat` falls outside `map.getBounds()` (+10% margin), invoked via the existing rAF on `move` and on `moveend`. `.cc-marker` and `.cc-geo-cluster` gain `will-change: transform; contain: layout style` for GPU compositing. Marker DOM stays in `markersRef` / `placeMarkersRef` so deconfliction/leader-lines/bubble expansions keep working. **Full DOM-markers → symbol-layer rewrite intentionally deferred** to a dedicated batch — see DECISIONS. 701 tests (+4 new tests for the pruner); `tsc` 0 errors; `next lint --dir src` clean. |
...
### Batch 14a validation (P1 rsvp-and-comments)
- `npx tsc --noEmit`: 0 errors
- `npx vitest run`: 78 files / **704 tests passing** (+3 RSVPButton error-surface tests)
- `npx next lint --dir src`: clean
- `mcp_supabase_get_advisors` (security): **84 WARN → 83 WARN** — `function_search_path_mutable` on `public.safe_rsvp` closed. No new warnings. The remaining anon/authenticated `_security_definer_function_executable` listings on safe_rsvp are PostgREST-exposure informational; the IDOR escalation path is closed by the in-function `auth.uid() = p_user_id` guard.
- Architect (SE: Architect): ✅ ship as-is. 6 Nice-to-haves logged (no Must-fix / Should-fix).
- Surface `rsvp-and-comments` → `clean`. Next P1 = `storage-and-media-uploads` (was P2).

### Batch 14b validation (P1 storage-and-media-uploads)
- `npx tsc --noEmit`: 0 errors
- `npx vitest run`: 78 files / **703 tests passing** (−1: SVG-skip test in `imageCompression.test.ts` removed alongside the SVG branch in `SKIP_TYPES`).
- `npx next lint --dir src`: clean
- `mcp_supabase_get_advisors` (security): **83 → 83** — code-only batch, no DB change.
- Architect (SE: Architect): ✅ ship as-is. **No Should-fix.** 4 nice-to-haves folded into Polish Queue row 10 / surface 17 deferred storage-hygiene line: (a) one-off purge of legacy `.svg` / `image/svg+xml` objects + null orphan URL refs, (b) extract shared `IMAGE_ACCEPT` constant from `validation.ts` (6 upload sites duplicate the literal), (c) future migration to dedicated `avatars` bucket, (d) ProfileEditor `accept=` attribute ordering cosmetic.
- Surface `storage-and-media-uploads` → `clean`. Next P1 = `auth-and-signup`.

### Batch 14c validation (P1 auth-and-signup)
- `npx tsc --noEmit`: 0 errors
- `npx vitest run`: 78 files / **703 tests passing** (unchanged — code-path tightening, no test surface change).
- `npx next lint --dir src`: clean
- `mcp_supabase_get_advisors` (security): **83 → 83** — code-only batch, no DB change.
- Architect inline review: ✅ ship as-is. **No Should-fix.** One nice-to-have: tighten `appliesTo` type from `string` to `(typeof VALID_APPLIES_TO)[number]` for compile-time enforcement at usage sites. Deferred — cosmetic.
- Security inline review (OWASP A03 Injection + A10 Open Redirect): no findings. `safeRedirect()` rejects `//` protocol-relative, `\\`, and any `:` (blocks `javascript:` / `data:` / `http:` etc.); allowlist on `applies_to` defenses-in-depth against PostgREST filter injection on `indemnity_templates`; the `redirect=` flow defense already existed on `auth/callback`'s `next` param — this matches that pattern for the email/password path.
- Surface `auth-and-signup` → `clean`. Next P1 = `edge-functions`.
