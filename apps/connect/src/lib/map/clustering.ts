 ...
 *
 * Tiers (user-confirmed zoom thresholds — see DECISIONS):
 *   capital  zooms 0–6
 *   city     zooms 6–9
 *   town     zooms 9–12
 *   suburb   zooms 12–15
 *   markers  zooms 15–16+
 *
 * Tiers crossfade with a 1.5-zoom overlap so the transitions feel smooth
 * instead of snap-swapping between grouping levels.
 *
...
export type ClusterTier = "capital" | "city" | "town" | "suburb";
 ...
 *  capital ≈ 400 km, city ≈ 100 km, town ≈ 20 km, suburb ≈ 5 km.
 ...
 * `[fullStart, fullEnd]` core and crossfades over `fadeWidth` zoom levels
 * on either side.  `marker` tier has no bubble — it's the individual
 * event/place markers owned by the existing render pipeline.
 *
 * Invariant: every zoom in [0, MARKER_FADE_IN_END] has at least one tier
 * at opacity > 0 so the map never looks "empty".
 */
const TIER_BANDS: Record<ClusterTier, { fullStart: number; fullEnd: number }> =
  {
    capital: { fullStart: 0, fullEnd: 5 },
    city: { fullStart: 6, fullEnd: 8 },
    town: { fullStart: 9, fullEnd: 11 },
    suburb: { fullStart: 12, fullEnd: 14 },
  };

/** Zoom levels below which markers are invisible / above which fully visible. */
export const MARKER_FADE_IN_START = 14;
export const MARKER_FADE_IN_END = 15.5;

const FADE_WIDTH = 1.5;
...
/** Quantise a lat/lng to a grid-cell origin for the given tier. */
...
export function visibleTiersAt(zoom: number): ClusterTier[] {
  const all: ClusterTier[] = ["capital", "city", "town", "suburb"];
