 ...
 * file: dependency because Connect deploys from this repo alone — see
 * vendor/README.md):
 *
 ...
 *  2. Smoke check — the vendored pipeline actually runs with Connect's own
...
describe("vendored @citizens/frontend-build", () => {
  ...
  it("runs the pipeline with Connect's own esbuild (injection smoke test)", () => {
    const { buildFrontend } = require("@citizens/frontend-build");
    const esbuild = require("esbuild");

    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "cc-frontend-build-"));
    try {
