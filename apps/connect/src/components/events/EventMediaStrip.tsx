...
/**
 * Thin, horizontally-scrollable strip of event photos + videos with a
 * ~60% opacity chrome. Clicking any tile opens a full-screen lightbox
 * overlay with keyboard/tap navigation and a close button.
 *
 * Designed to sit inside the event detail page — it degrades to nothing
 * when `media` is empty so callers can always render it.
 */
        ...
        >
          {media.map((m, i) => (
            <button
              ...
              type="button"
              onClick={() => open(i)}
              aria-label={m.title ? `Open ${m.title}` : `Open gallery item ${i + 1}`}
              className="relative h-20 w-28 flex-shrink-0 overflow-hidden rounded-lg bg-black/5 shadow-sm transition hover:ring-2 hover:ring-(--gold) focus:outline-none focus:ring-2 focus:ring-(--gold)"
