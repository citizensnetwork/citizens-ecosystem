...
| **4b — DOCS SHIPPED (2026-07-02) · mig 145 ⛔ awaiting founder** | **Ecosystem profile-levels contract** delivered docs-first: **[`docs/ECOSYSTEM_PROFILE_LEVELS.md`](../ECOSYSTEM_PROFILE_LEVELS.md)** (normative: Citizen → per-app creating tier [Connect Contributor lifecycle · Wear Creator/Brand · Vision `user_org_roles` RBAC] → per-app Admin; P-rules: no cross-app inheritance, no super-role, DB-layer self-escalation blocks) + SHARED_DB_CONTRACT **R6.3/R6.4** amendment. The one gap = **Wear admin/moderation**: proposed as draft **[`docs/wear/145_wear_admin_moderation.sql`](../wear/145_wear_admin_moderation.sql)** (`wear.user_roles` service_role-managed + `is_moderator()`/`is_admin()` + reports triage lifecycle + takedown policies, DMs excluded) — **NOT applied; founder must confirm before prod apply** (tag → apply → advisors 0 ERROR → §9 re-stamp). | founder request 2026-07-02 |
