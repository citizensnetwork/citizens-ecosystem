...
| **4 ✅ SHIPPED (2026-07-02)** | **`@citizens/frontend-build` extracted** — canonical source `citizens-wear/packages/frontend-build` (workspace pkg, 19 tests, 100% line cov); Wear consumes via `workspace:*`, Connect via a vendored `file:` dep (`vendor/citizens-frontend-build` + `npm run sync:frontend-build` + vitest drift-guard) since Connect deploys alone on Vercel. Host apps inject their **own esbuild** → both apps' `public/`+`mobile-dist/` outputs verified **byte-identical** pre/post (SHA-256 tree compare). Vision becomes the 3rd consumer at step 4c (same vendoring pattern until step 5). **Other pure-TS candidates assessed — deliberately deferred:** `@citizens/utils`(rate-limit) extracts WITH the Wear rate-limiting fast-follow (first real 2nd consumer — don't ship an unconsumed package); `@citizens/contracts` + `connect-client` sharing re-evaluate at 4c when Vision's `/api/v1` client is touched; `@citizens/db` types stay per-app (sibling apps consume Connect via `/api/v1` per contract R2 — shared raw-table types would invite contract violations). | D5 · §5 |
| **4b ✅ COMPLETE (2026-07-02) — mig 145+146 APPLIED (founder-confirmed)** | **Ecosystem profile-levels contract** delivered docs-first: **[`docs/ECOSYSTEM_PROFILE_LEVELS.md`](../ECOSYSTEM_PROFILE_LEVELS.md)** (normative: Citizen → per-app creating tier [Connect Contributor lifecycle · Wear Creator/Brand · Vision `user_org_roles` RBAC] → per-app Admin; P-rules: no cross-app inheritance, no super-role, DB-layer self-escalation blocks) + SHARED_DB_CONTRACT **R6.3/R6.4** amendment. The Wear admin/moderation gap **closed in prod**: migration **145** (`wear.user_roles` service_role-managed + `is_moderator()`/`is_admin()` + reports triage lifecycle + takedown policies, DMs excluded) + **146** (grants fix surfaced by the post-apply smoke: `user_roles` had no table-level grants — `authenticated` SELECT-only + `service_role` full, least-privilege). Advisors 0 ERROR / 0 new; moderator round-trip smoke-verified via rolled-back prod transactions; contract §9 re-stamped (head 146). Remaining app-side: Wear `/api/admin/*` + triage screen (fast-follow list). | founder confirmed 2026-07-02 |
| **4c ✅ SHIPPED (2026-07-02)** | **Vision reconcile + HTML frontend** executed per **[`STEP4C_VISION_RECONCILE_SCOPE.md`](./STEP4C_VISION_RECONCILE_SCOPE.md)** (citizens-vision e39aa88 → `3602a86`, 3 gated increments). **Unit 2:** day-one rate limiting (Connect's limiter ported byte-compatible + blanket per-IP `/api/*` gate in the proxy) + Bearer-token auth delivered in ONE place (`lib/supabase/server.ts` — all 45 handlers gained cross-origin auth with zero route edits). **Unit 3:** standalone HTML frontend (`src/frontend/`, 8 screens incl. the **login page**) as `@citizens/frontend-build`'s **3rd consumer** (Connect vendoring pattern + drift test, `__CV_ENV`, stub capacitor-bridge — desktop back-office, no mobile build); design source = the **2026-07-02 `Citizens Vision Design.zip` handoff** (`_ds` tokens + `VISION_BUILD_PLAN.md`, extracted to App Planning Docs/Vision/design-handoff-20260702) — **Q1 resolved** (build-from-plan, no fuller export needed); the **narrative engine** (§4 ⭐) ships: all insight copy is `{template,data}` slots = the backend calc contract. **Unit 4 (Q3 reconfirmed):** RSC tree deleted (149 files), Next.js **API-only**, `/` → `/index.html`, CSP retuned for the static model (Vision = the only Citizens app with a CSP). Demo→live wiring of the 45 handlers into the screens = the remaining Vision fast-follow. Vision DDL numbers from **147** (146 = Wear grants fix). | founder confirmed 2026-07-02 |
