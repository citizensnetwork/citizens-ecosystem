...
| **4c — SCOPED (2026-07-02) · execution awaits founder Q1–Q3** | **Vision reconcile + HTML frontend** — scope locked in **[`STEP4C_VISION_RECONCILE_SCOPE.md`](./STEP4C_VISION_RECONCILE_SCOPE.md)** (Wear's §6a sequence adapted: wiring-spec units → `/api/*` audit of the 45 existing handlers + Bearer route-context + day-one rate limiting → HTML swap as `@citizens/frontend-build`'s 3rd consumer via the Connect vendoring pattern → Next.js API-only). **Design-asset finding:** `Citizens Vision.zip` is a 353 KB Claude-design *canvas/reference* (dc.html + 5 screenshots + the known PDFs/MDs), **NOT an importable app-source handoff** like Wear's — founder must choose: fuller design export vs build-from-canvas (scope §5 Q1). Vision DDL numbers from **146** (145 reserved for the Wear admin draft). | founder request 2026-07-02 · §6a precedent |
