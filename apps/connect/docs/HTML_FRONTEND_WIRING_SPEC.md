...
- **Pin labels (2026-08-26):** a pin's name floats on a fuzzy white *mist*
  ...
  capsule, set in the app's own type at 12px/800. Shown from z 16.5 up (tuned across the
  original 12.6 → 15.6 → 18 → 16.5 — 18 turned out too tight, labels almost never showed
  at normal browsing zoom), plus always for the selected pin. Styles live in `index.html`
  so the zoom gate toggles them with one attribute (`.cc-map[data-cc-labels]`).
