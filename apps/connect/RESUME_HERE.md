...
- 🟡 **[P4] event-create-edit** — 2 staged fixes (audited 2026-05-22), run `/audit-fix event-create-edit`. Checkpoint: `.audit/surfaces/event-create-edit.md`. Patches: boundary validation (title/coords/end-after-start), surface delete errors in EditEventForm.
- 🟡 **[P6] messaging-dm** — 3 staged fixes (audited 2026-05-22), run `/audit-fix messaging-dm`. Checkpoint: `.audit/surfaces/messaging-dm.md`. Patches: `.maybeSingle()` parity for participant/recipient/cursor lookups, PATCH /read rate-limit, GET /messages `limit` NaN guard. All low-risk; RLS posture clean.
