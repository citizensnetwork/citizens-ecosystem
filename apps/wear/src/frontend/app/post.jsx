...
(function () {
  ...
  const { GOLD, INK, Avatar, BrandLogo, Spinner, ErrorNote, ScreenHeader, timeAgo, fmtCount } =
    window.CWUI;
  ...
  function PostScreen({ params }) {
    ...
    const toggleLike = async () => {
      try {
        ...
        setState((s) => ({
          ...s,
          data: { ...s.data, viewerLiked: res.liked, likeCount: res.likeCount },
        }));
      } catch (e) {
        /* leave state */
      }
    ...
    return h(
      ...
      h(
        'div',
        { className: 'cwsc', style: { flex: 1, overflowY: 'auto', paddingBottom: 10 } },
        // author
        h(
          'div',
          { style: { display: 'flex', alignItems: 'center', gap: 11, padding: '6px 18px 12px' } },
          h(
            'button',
            {
              onClick: () =>
                post.brand
                  ? openBrand(post.brand.slug)
                  : post.author && openUser(post.author.handle),
              style: { border: 'none', background: 'none', padding: 0 },
            },
            post.brand
              ? h(BrandLogo, { brand: post.brand, size: 40 })
              : h(Avatar, { user: post.author, size: 40 }),
          ),
          h(
            'div',
        ...
        h(
          ...
          h(
            ...
            Icon('heart', {
              size: 25,
              color: viewerLiked ? GOLD : INK,
              fill: viewerLiked ? GOLD : null,
            }),
            h(
              'span',
              { style: { fontSize: 12.5, fontWeight: 700, color: viewerLiked ? GOLD : INK } },
              fmtCount(likeCount),
            ),
          ),
          h(
          ...
          h(
            ...
            Icon('bookmark', {
              size: 23,
              color: viewerSaved ? GOLD : INK,
              fill: viewerSaved ? GOLD : null,
            }),
          ),
        ),
        // comments
        h(
          ...
          comments.length === 0
            ? h(
                'div',
                { style: { fontSize: 12.5, color: '#a09e97', fontWeight: 500 } },
                'Be the first to encourage.',
              )
            : comments.map((c) =>
                h(
                  'div',
                  { key: c.id, style: { display: 'flex', gap: 10, padding: '8px 0' } },
                  ...
                  h(
                    ...
                    h(
                      'div',
                      {
                        style: { fontSize: 10.5, color: '#c2c0b9', fontWeight: 600, marginTop: 3 },
                      },
                      timeAgo(c.createdAt),
                    ),
                  ),
