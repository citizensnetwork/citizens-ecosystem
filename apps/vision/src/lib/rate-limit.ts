 ...
 *
 * Ported from Citizens Connect (`src/lib/rate-limit.ts`) as part of ecosystem
 * Step 4c — Vision ships rate limiting from DAY ONE (the lesson from Wear
 * debt #1: its `/api/*` surface launched without any). The implementation is
 * kept byte-compatible with Connect's so the planned `@citizens/utils`
 * extraction (decision brief row 4) can consolidate the two verbatim.
 *
 ...
 * REST API over `fetch`, no SDK dependency).
 *
 ...
 * fallback keeps a true sliding-window (per-timestamp) behaviour.
 */
...
export const RATE_LIMITS = {
  ...
  /** Heavy operations (exports, advisory generation): 5 per minute */
