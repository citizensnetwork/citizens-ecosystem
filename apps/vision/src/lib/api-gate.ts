/**
 * Blanket `/api/*` rate-limit gate, called from the proxy (middleware) so
 * every one of the ~45 route handlers is covered from day one without
 * per-route wiring. Individual hot routes can ADD a tighter per-route
 * `checkRateLimit` later (Connect's pattern); this gate is the floor that
 * guarantees no endpoint ships unlimited.
 *
 * Identity is the caller IP (Vercel's `x-forwarded-for`), split by method
 * class so a burst of dashboard reads cannot starve a legitimate write:
 *   GET/HEAD     → 240/min per IP  (org dashboards fan out many parallel reads)
 ...
 */
export async function gateApiRequest(request: HeaderCarrier & { method: string }): Promise<GateResult> {
